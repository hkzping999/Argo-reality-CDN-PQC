...
VERSION='0.7.1-stable-first-pqc-1rtt'
...
prepare_pqc(){
  ...
  if [[ "$VLESS_PQC_DECRYPTION" == ${VLESS_PQC_REQUIRE_PREFIX}.* && "$VLESS_PQC_ENCRYPTION" == ${VLESS_PQC_REQUIRE_PREFIX}.* ]]; then
    ...
    if truthy "${VLESS_PQC_DISABLE_0RTT:-y}" && { [[ "$VLESS_PQC_DECRYPTION" == *'.0rtt.'* ]] || [[ "$VLESS_PQC_ENCRYPTION" == *'.0rtt.'* ]]; }; then
      VLESS_PQC_DECRYPTION=''; VLESS_PQC_ENCRYPTION=''
      truthy "$VLESS_PQC_STRICT" && fatal 'PQC 0-RTT 禁用失败。'
      warn 'PQC profile 未启用：检测到 0-RTT 参数。兼容节点继续部署。'
    else
      PQC_READY=y
    fi
  else
...
normalize_vless_pqc_server_ticket(){
  ...
  if truthy "${VLESS_PQC_DISABLE_0RTT:-y}"; then
    printf '%s' "$v" | sed -E "s/^(mlkem768x25519plus\.[^.]+\.)(0rtt|1rtt|[0-9]+s|[0-9]+-[0-9]+s)\./\1${resume}./"
  else
    printf '%s' "$v"
  fi
}
normalize_vless_pqc_client_ticket(){
  ...
  if truthy "${VLESS_PQC_DISABLE_0RTT:-y}"; then
    printf '%s' "$v" | sed -E "s/^(mlkem768x25519plus\.[^.]+\.)(0rtt|1rtt|[0-9]+s|[0-9]+-[0-9]+s)\./\1${rtt}./"
  else
    printf '%s' "$v"
  fi
}
